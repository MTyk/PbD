...
using namespace boost::filesystem;
	...
	Controller::Controller(std::string const& name) : TaskContext (name), prepared {false}, execute {false}, executor {nullptr}, error() {
		...
		this->addEventPort("haltExecution", port_halt_executor, boost::bind(&Controller::haltExecution,this,_1)).doc("Stop the executor");
...
// Setup maximum vector size
	...
	void Controller::stopExecution(RTT::base::PortInterface*) {
		Logger::In in((this->getName()));
		log(Info) << "Controller stopped" << endlog();
		this->stop();
	}
	//added by Martin Tykal
	void Controller::haltExecution(RTT::base::PortInterface*) {
		Logger::In in((this->getName()));
		log(Info) << "Execution stopped" << endlog();
		execute=false;
	}
