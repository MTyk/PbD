...
#define ASSISTANCE 0
	...
	KUKAInterfacePoseForceZ::KUKAInterfacePoseForceZ(string nodeName) : KUKAInterface(nodeName, FRI_CTRL_CART_IMP)	{
		...
		double stiffnessValue = 2000;
		double dampingValue = 200;
		if (ASSISTANCE) {
	...
	void KUKAInterfacePoseForceZ::approach_state() {
		...
		double f_approach = 1;
		double f_max_no_contact = 0.5;

		// Force Z or Position Z control?
		if(fabs(f_z_des) > 0.5 // Force control, when desired force is bigger than 0.5N
			...
			if(wait_for_contact && fabs(f_ft_z) < f_max_no_contact /*&& fabs(f_z_des) > f_max_no_contact*/) { 	// Not in contact yet
				...
				static double P_vel = 50;	//50		// using PID controller
				static double I_vel = 10;	//300	
				...
				static double P_f = 0; //0 //0.2;//0.05;			// using PID controller
				static double I_f = 0.8;//0.8//0.5;//0.1;
				static double D_f = 0.002;//0.002//0.005;//0.01;
				static double last_f_e = 0;
				static double f_e_accum = 0; //200  // experimental value
				...
				//f.force.z = f_z_des + (f_cur.force.z - f_ft_z);
				last_f_e = f_e;
	...
	*/
		...
		if(force_ctrl) {	// Ignore pose while in force control
			...
			if(wait_for_contact)
				// Stop evolution
				state_msr[7] = state[7] * 10;//+ (f_z_des - f_ft_z) / 4;
				//state_msr[7] = state[7] + (f_cur.force.z - f_z_des) / 4;
	...
	void KUKAInterfacePoseForceZ::updateHook() {
		...
		if(check_start_state) {
			...
			double min_rot = 0.01;
			double angle = 2 * acos(aw*bw + (ax*bx + ay*by + az*bz));
			angle =  angle > M_PI ? -(2 * M_PI - angle) : angle;
